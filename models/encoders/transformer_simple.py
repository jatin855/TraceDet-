...
class TransformerMVTS(nn.Module):
    ...
    def __init__(self, 
            ...
            trans_dropout = 0.1, # Dropout rate in Transformer encoder
    ...
    def embed(self, src, times, static = None, captum_input = False,
        ...
        if aggregate:
            ...
            elif self.aggreg == "last_token":
                if src_mask is not None:
                    lengths = (1 - src_mask).sum(dim=1)  
                    lengths = lengths.clamp(min=1) 
                    idx = (lengths - 1).unsqueeze(0).unsqueeze(-1).repeat(1, 1, output_preagg.shape[-1]) 
                    output = output_preagg.gather(0, idx).squeeze(0) 
                else:
                    output = output_preagg[-1, :, :] 
    ...
    def forward(self, 
        ...
        * Ensure all inputs are cuda before calling forward method
        ...
        out = nn.LayerNorm(out.size(-1)).to(out.device)(out)
        output = self.mlp(out)
