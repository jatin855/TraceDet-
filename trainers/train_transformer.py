...
from models.run_model_utils import batch_forwards_TransformerMVTS
from models.encoders.simple import CNN, LSTM
...
def train(
    ...
    for epoch in range(num_epochs):
        ...
        for i, (X, times, y) in enumerate(train_loader):
            # print(X.shape, times.shape, y.shape)#torch.Size([64, 200, 4]) torch.Size([64, 200]) torch.Size([64])
        ...
        if (epoch + 1) % print_freq == 0: # Print progress:
            ...
            print('Epoch {}, Train Loss = {:.4f}, AUC = {:.4f}'.format(epoch + 1, train_loss[-1], auc))
    ...
    print('Best AUC achieved at Epoch = {}, Val_AUC = {:.4f}'.format(best_epoch, max_val_auc))
